...
( function($, window, document, JSON, _ns) {"use strict";
        ...
        _ns.Controller = function(_i18n, _model, _view, _api, _cometd) {
            ...
            _view.pages.admin.onApplyCliAppAuth =  function() {
               ...
               _fillConfigPropsYN(props, ['cliapp.auth.trust-user-account', 
                    'cliapp.auth.trust-webapp-user-auth']);
            ...
            _view.pages.admin.onApplySmartSchool = function(enable1, enable2) {
                ...
                _fillConfigPropsYN(props, [
                    'smartschool.user.insert.lazy-print'    
                ]);
            ...
            _view.pages.admin.onApplySmartSchoolPaperCut = function(enable) {
                ...
                if (enable) {
                    _fillConfigPropsText(props, [
                        ...
                        'papercut.webservices.auth-token',
                        'papercut.db.jdbc-url',
                        'papercut.db.user',
                        'papercut.db.password'
                        ]);
                }
                _saveConfigProps(props);
            };
            ...
            _view.pages.admin.onApplyProxyPrint = function() {
                ...
                _fillConfigPropsYN(props, ['proxy-print.non-secure', 
                    'webapp.user.proxy-print.clear-inbox']);
                
                if (props['proxy-print.non-secure'] === 'Y') {
                    _fillConfigPropsText(props, ['proxy-print.non-secure-printer-group']);
                }
                
                _fillConfigPropsText(props, ['proxy-print.fast-expiry-mins', 
                    'proxy-print.hold-expiry-mins',
                    'proxy-print.direct-expiry-secs',
                    'webapp.user.proxy-print.max-copies']);
            ...
            _view.pages.admin.onApplyFinancialGeneral = function() {
                ...
                //
                ;
                _fillConfigPropsText(props, ["financial.global.credit-limit", 
                    "financial.printer.cost-decimals", "financial.user.balance-decimals"]);
            ...
            _view.pages.admin.onApplyFinancialPos = function() {
                ...
                //
                ;
                _fillConfigPropsText(props, ["financial.pos.payment-methods", 
                    "financial.pos.receipt-header"]);
                _saveConfigProps(props);
            };
            
            _view.pages.admin.onApplyFinancialVouchers = function(cardFontFamily) {
                var props = {"financial.voucher.card-font-family" : cardFontFamily};
                _fillConfigPropsText(props, ["financial.voucher.card-header", 
                    "financial.voucher.card-footer"]);
                _saveConfigProps(props);
            };
            ...
            _view.pages.admin.onApplyImap = function(enabled, security) {
                ...
                if (enabled) {
                    ...
                    _fillConfigPropsText(props, ['print.imap.host', 'print.imap.port', 
                        'print.imap.user', 'print.imap.password', 
                        'print.imap.folder.inbox', 'print.imap.folder.trash', 
                        'print.imap.max-file-mb', 'print.imap.max-files']);
                }
                _saveConfigProps(props);
            };
            ...
            _view.pages.admin.onApplyUserAuthModeLocal = function() {
                var props = {};
                _fillConfigPropsYN(props, ['auth-mode.name', 'auth-mode.name.show', 
                    'auth-mode.id', 'auth-mode.id.show', 'auth-mode.id.is-masked', 
                    'auth-mode.id.pin-required', 'auth-mode.card-local', 
                    'auth-mode.card-local.show', 'auth-mode.card.pin-required', 
                    'auth-mode.card.self-association', 
                    'user.can-change-pin', 'webapp.user.auth.trust-cliapp-auth']);
                _fillConfigPropsRadio(props, ['auth-mode-default', 'card.number.format', 
                    'card.number.first-byte']);
                _saveConfigProps(props);
            };
                ...
                if (res.result.code === '0') {
                    ...
                    if (_model.editUser.dbId) {
                        if (_api.call({
                            ...
                            dto : JSON.stringify({ key : _model.editUser.dbId})
                        }).result.code !== '0') {
                            _view.showApiMsg(res);
                        }
                    }
        ...
        _ns.Model = function(_i18n) {
            ...
            this.pubMsgAsHtml = function(data) {
                var sfx = (data.level === "ERROR") ? "error" : (data.level === "WARN") ? "warn" : 
                    (data.level === "INFO") ? "info" : "valid";
